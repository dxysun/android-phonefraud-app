...
//    protected CallRecord callRecord;
 ...
 /*   public CallRecordReceiver(CallRecord callRecord) {
    ...
    protected void onIncomingCallReceived(Context context, String number, Date start) {
       ...
       // Toast.makeText(context.getApplicationContext(), "onIncomingCallReceived", Toast.LENGTH_LONG).show();
            phonenumber = number;
 ...
 //       SpeechApp appState = ((SpeechApp)context.getApplicationContext());
  //      appState.setPhonenumber(number);
    ...
    protected void onIncomingCallAnswered(Context context, String number, Date start) {
        //startRecord(context, "incoming", number);
       // Toast.makeText(context.getApplicationContext(), "onIncomingCallAnswered", Toast.LENGTH_LONG).show();
        cancelToast();
    ...
    protected void onIncomingCallEnded(Context context, String number, Date start, Date end) {
        Log.i(TAG,"onIncomingCallEnded   end" );
        stopRecord(context);
        cancelToast();
        ///   Toast.makeText(context.getApplicationContext(), "onIncomingCallEnded", Toast.LENGTH_LONG).show();
   ...
   /*     RecordTask   recordertask = new RecordTask();
    ...
    protected void onOutgoingCallEnded(Context context, String number, Date start, Date end) {
        Log.i(TAG,"onOutgoingCallEnded   end" );
        stopRecord(context);
    }
    ...
    //    Toast.makeText(context.getApplicationContext(), "onMissedCall", Toast.LENGTH_LONG).show();
        cancelToast();
    }

    // Derived classes could override these to respond to specific events of interest
    protected void onRecordingStarted(Context context) {
    //    SpeechApp appState = ((SpeechApp)context.getApplicationContext());
    //    appState.setSavefilePath(audiofile.getAbsolutePath());
    }

    protected void onRecordingFinished(Context context) {
    }
 ...
 /*           String file_name = PrefsHelper.readPrefString(context, CallRecord.PREF_FILE_NAME);
        ...
        //    int output_format = PrefsHelper.readPrefInt(context, CallRecord.PREF_OUTPUT_FORMAT);
         ...
         //   int audio_encoder = PrefsHelper.readPrefInt(context, CallRecord.PREF_AUDIO_ENCODER);*/
            String dir_path = "/storage/emulated/0";
            String dir_name = "PhoneCallRecorder";
            String file_name = "CallRecorder";
            int audio_source = MediaRecorder.AudioSource.VOICE_COMMUNICATION;
            boolean show_phone_number = true;
            boolean show_seed = true;
